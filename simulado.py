...
def gerar_docx_banco(df_filtrado):
    ...
    for idx, (_, r) in enumerate(df_filtrado.iterrows(), 1):
        ...
        if r.get("Link Imagem"):
            doc.add_paragraph(f"Link da Imagem: {r.get('Link Imagem')}")
            
        for l in ["A", "B", "C", "D", "E"]:
            doc.add_paragraph(f"({l}) {limpar_texto(r.get(l))}")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if senha == SENHA_COORD:
        ...
        if df is not None and not df.empty:
            ...
            with f1:
                sel_turma = st.multiselect("Filtrar Turma:", sorted(df["Turma"].unique()))
            with f2:
                sel_disc = st.multiselect("Filtrar Disciplina:", sorted(df["Disciplina"].unique()))
            with f3:
                sel_prof = st.multiselect("Filtrar Professor(a):", sorted(df["Professor (a)"].unique()))
            ...
            st.metric("Questões Encontradas", len(df_v))
            st.dataframe(df_v, use_container_width=True, hide_index=True)

            st.divider()
            st.markdown("### ⬇️ Exportar Banco Selecionado")
            ...
            with exp1:
                if st.button("📄 Gerar Banco em PDF", use_container_width=True):
                    ...
                    for i, (_, r) in enumerate(df_v.iterrows(), 1):
                        ...
                        pdf.multi_cell(0, 6, f"QUESTÃO {i:02d} - {r['Turma']} ({r['Disciplina']})")
                        pdf.set_font("Arial", "", 11)
                        pdf.multi_cell(0, 6, limpar_texto(r['Pergunta']))
                        pdf.ln(2)
                        for l in ["A", "B", "C", "D", "E"]:
                            pdf.multi_cell(0, 6, f"({l}) {limpar_texto(r[l])}")
                        pdf.ln(4)
                    st.download_button("⬇️ Baixar PDF", data=pdf.output(dest='S').encode('latin-1', 'replace'), file_name="banco_side.pdf", use_container_width=True)

            with exp2:
                # --- OPÇÃO DOCX RESTAURADA ---
                docx_banco = gerar_docx_banco(df_v)
                st.download_button(
                    label="⬇️ Baixar Banco em Word (Docx)",
                    data=docx_banco,
                    file_name=f"Banco_SIDE_{datetime.now().strftime('%Y%m%d')}.docx",
